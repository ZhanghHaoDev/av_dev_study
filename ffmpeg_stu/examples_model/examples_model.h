...
public:
    ...
    int avio_read_callback(std::string file_pact);

    // av_dump_format 的使用
    int my_av_dump_format(std::string file_path);

    // mp2 文件解码为 PCM文件
    int decode_audio_mp2_pcm(std::string file_path,std::string output_file);
